...
|---|---|---|
| v4.3.1 | 2026-08-24 | Fix en la resolución de `<último-release>`, detectado al correr `/CBP` sobre la propia Diligencia: se buscaba solo por mensaje de commit, pero los releases v4.2.0–v4.3.0 se marcaron con tag sobre el commit de trabajo, sin commit de release propio — la resolución no los veía y reportaba 12 commits "sin liberar" ya publicados. Ahora resuelve a la más reciente de dos señales: último commit de release **o** commit del último tag. |
| v4.3.0 | 2026-08-24 | Cierra M3/M4/M5 de Nemesis. **`shell-lock.json`**: manifiesto de huellas de `~/.claude/` (45 archivos) — `/CBP` detecta solo qué cambió del shell sin versionar, sin depender de que alguien lo anote. **`/salud` 1h**: integridad de IDs del ROADMAP. **`/salud` 1i**: pendientes con señal de estar ya hechos. **`MECANICA-CALIDAD` §1**: la tabla Completado conserva el ID — descartarlo garantizaba referencias colgadas. |
...
| v3.10.2 | 2026-07-31 | R6 refinada: criterios explícitos de bump. Pre-check en CBP. Incidente ICT-DIL-20260731-02. |
| v3.10.1 | 2026-07-31 | Observabilidad: triggers de monitoreo. Troubleshooting. Agents opencode. |
| v3.10.0 | 2026-07-31 | CRITICO: fix opencode.jsonc schema (ICT-DIL-20260731-01). Refs docs. |
| v3.9.2 | 2026-07-30 | VAIO: 45 commits automatizados de watchdog (heartbeats + URL updates). Sin cambios funcionales. |
