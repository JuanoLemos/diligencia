...
| R79.1 | Burn rate discipline (2026-07-30): tras incidente de USD 10/día por loop `check-tareas` + `publish-url` con `deepseek-v4-pro`, todas las scheduled tasks IA requieren **creación manual** via `scripts/register-task.ps1` (rechaza modelos `pro`, `claude`, `sonnet`, `opus`, `gpt-4`, `gemini-pro`). `start-chamber.ps1` ya NO recrea tasks automáticamente. Health checks y publicación de URL se realizan con scripts PowerShell puros (sin LLM): `scripts/check-tareas.ps1`, `doc/vaio/watchdog-tunnel.ps1`. Toda invocación de LLM debe pasar por `invoke-agent-task.ps1` con bootstrap lazy + scope filter. |
