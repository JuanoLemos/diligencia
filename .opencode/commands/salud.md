...
# /salud — Cuidado integral del proyecto
...
DETECTAR qué workflow invocó al doctor (/CBP doctor, /CBP completo, /CBP updoc, standalone /salud)
...
CALCULAR: "Se creará backup #<N+1> en `.old/salud-backups/`. Pruning: se conservan últimos $BACKUP_KEEP backups."
...
4. DETECTAR workflow: /CBP doctor, /CBP completo, /CBP updoc, o standalone /salud
5. CALCULAR número de backup: contar filas de tabla en $BACKUPS + 1
6. CREAR directorio: `.old/salud-backups/backup-<N>-<YYYYMMDD-HHMM>/`
...
8. AGREGAR entrada en $BACKUPS:
   ...
   ```
9. PRUNING: si total backups en `.old/salud-backups/` > $BACKUP_KEEP:
...
10. REPORTAR: "✅ Backup #<N> creado: `.old/salud-backups/backup-<N>-<timestamp>/` (<cantidad> archivos)"
...
### 3f — Post-corrección
- AGREGAR entrada en CHANGELOG.md en sección "Changed" o nueva entrada no versionada: "- `/salud`: <N> correcciones aplicadas"
- Si hubo cambios en AGENTS.md o estructura: SUGERIR commit message `chore: /salud — <N> correcciones en estructura, código, tracking`
- AUTOCIERRE: Si `/salud` está como `🔴 Pendiente` en RM "Next" → actualizar a `✅ Hecho`. Si está `[ ]` en CHECKLIST → marcar `[x]`.
...
**💾 Backup pre-corrección:** backup #N creado en `.old/salud-backups/`, pruning aplicado si > $BACKUP_KEEP
...
- /health no aborta /salud si stack ≠ JS (reporta ⚠️ y continúa)
...
- Autocierre aplicado: /salud marcado como ✅/[x] en RM/CHECKLIST si estaba pendiente
...
- `.old/salud-backups/` (directorio de backups + pruning)
...
## Anti-patrones
- NO abortar /salud si una sub-verificación falla o no aplica — CONTINUAR con la siguiente
...
- NO eliminar backups manuales o no generados por /salud durante el pruning
